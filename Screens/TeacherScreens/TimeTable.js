import { ImageBackground, StyleSheet, Text, View, Dimensions, TouchableOpacity, Image, FlatList } from 'react-native'
...
import { getTimeTableforTeacher } from '../../store/slices/TimeTableSlice';
...
const TimeTable = () => {
    ...
    const { days, teacherTimeTable, error } = useSelector(state => state.timetable)
    ...
    const fetchTimeTable = () => {
        const { id } = UserData
        dispatch(getTimeTableforTeacher({ day, id }))
    }
                                ...
                                renderItem={({ item, index }) => (
                                    ...
                                    <View key={index} style={styles.list}>
                                        ...
                                        <Text style={{ fontWeight: "500", color: "black", fontSize: 16, alignSelf: "center" }}>
                                            {item.class_model?.name}-{item.section_model?.name}
                                        </Text>
